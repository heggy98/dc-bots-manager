...
{
    /// <summary>
    /// Handles team-related commands: /board add-team, /board remove-team, /board edit-team, /board refresh, /board insert, /board publish-board
    /// </summary>
    public class TeamsCommandHandler
    {
        ...
        {
            var invokedSubcommand = command.Data.Options?.FirstOrDefault()?.Name;
            var commandKey = string.Equals(invokedSubcommand, "insert", StringComparison.OrdinalIgnoreCase)
                ? "insert"
                : "publish-board";
            var boardCommand = await GetCommandAsync(commandKey, context);
